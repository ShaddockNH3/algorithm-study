...
比如说在英文中，e的使用频率大于z，传统编码（如阿斯克码），为每个字符分配相同长度的二进制编码，这会导致高频字符也占用较多的空间，而这些字符可以使用更短
的编码
...
1. 构建huffman树，将每个字符和其频率加入优先队列中，每次取出频率最低的两个结点，合成一个新节点，直到只剩下一个根节点，构成huffman树。

1. 从根节点触发，向左走编码为0，向右走编码为1，为每个字符生成唯一的编码
