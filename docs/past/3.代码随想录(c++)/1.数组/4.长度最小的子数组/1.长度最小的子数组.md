...
[209. 长度最小的子数组 - 力扣（LeetCode）][link-1]
...
```cpp
// 这是原先原来的指针移动逻辑，现在我们假设目标是“和必须等于target”
if (currnetsum == target) {
    minlength = min(minlength, currnetlength); // 和正好等于target，记录长度
...
[link-1]: https://leetcode.cn/problems/minimum-size-subarray-sum/description/
