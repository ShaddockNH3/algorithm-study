        ...
        for (int i = 0; i < nums.size(); i++) {
            ...
            if (!two_sum_result.empty()) {
                for (int j = 0; j < two_sum_result.size(); j += 2) {
                    vector<int> triplet = {
                        nums[i],
                        subarray[two_sum_result[j]],
                        subarray[two_sum_result[j + 1]],
                    };
...
[link-1]: https://leetcode.cn/problems/3sum/?envType=study-plan-v2&envId=top-100-liked
